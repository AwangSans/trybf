...
def get_centroid_of_max_n(fk, n):
    # 获取方阵的行数和列数
    rows, cols = fk.shape
    # 获取方阵的中心位置
    center_row = rows // 2
    ...
    # 扁平化数组并获取最大的n个值的索引
    flat_indices = np.argpartition(fk.flatten(), -n)[-n:]
    # 将扁平化索引转换为二维索引
    row_indices, col_indices = np.unravel_index(flat_indices, fk.shape)

    # 计算重心位置
    centroid_row = np.sum(row_indices * fk[row_indices, col_indices]) / np.sum(fk[row_indices, col_indices])
    centroid_col = np.sum(col_indices * fk[row_indices, col_indices]) / np.sum(fk[row_indices, col_indices])

    # 以中心为原点的重心位置矢量
    centroid_vector = np.array([centroid_row - center_row, centroid_col - center_col])
    slowness = np.sqrt((row_indices[0] - center_row) ** 2 + (col_indices[0] - center_col) ** 2)

    # 计算方位角
    azimuth = np.arctan2(centroid_vector[1], centroid_vector[0])
    # 将方位角转换为度，范围从 0 到 360 度
...
def single_saclist_process(arg_list):
    ...
    # np.save(fk_file, fk)
    n= 50
    azimuth,slowness = get_centroid_of_max_n(fk, n)
    
    return([fk.max(), azimuth, slowness])
...
def process_all_files_in_folder(folder_path, start_hour, target_rate, output_folder):
    ...
    three_hours_points = timepace * 60 * 60 
    start_points = start_hour * 60 * 60 
        ...
        for file in files:
            if file.endswith('.sac'):
                ...
                if downsampled_stream:
                    ...
                    processed_stream = downsampled_stream
...
if __name__ == '__main__':
    ...
    dl = 1
    Signal11=[]
    change_time = [140]#[i for i in range(0,12*22)]#0,8*22-->1:(0,2*22)2:(2*22,4*22)3:(4*22,6*22)4:(6*22,8*22)
    
    freq_list = [0.24]#np.linspace(0.02,0.48,24)# edited
    for freq2find in freq_list:
        cap_find = round(freq2find * nsamp *1)
        #for hour in range(8*22):
        for hour in change_time:
            folder_path = "data"
            timepace = 2
            start_hour = hour * timepace
            target_rate = 1
            output_folder = 'output'
            process_all_files_in_folder(folder_path, start_hour, target_rate, output_folder)

            sac_list_dir = './saclist_by_time'
            ...
            for sac_list in glob.glob(sac_list_dir + '/*.txt'):
                ...
                # 将当前小时信息加入输出文件名
                output_file = os.path.join(output_dir, f'{label}_hour_{start_hour}_fk.npy')
            ...
            for arg_list in arg_list_list:
                Signal11.append(single_saclist_process(arg_list))
        
    print(Signal11)
    np.save('answer.npy', Signal11)
